...
def booking_history():
    ...
    try:
        ...
        # สร้าง query สำหรับประวัติการจอง - แสดงการจองที่เสร็จสิ้นแล้ว
        query = '''SELECT b.booking_id, b.booking_date, b.status, c.first_name, c.last_name, v.license_plate, v.license_province
                   FROM bookings b
                   JOIN customers c ON b.customer_id = c.customer_id
                   JOIN vehicles v ON b.vehicle_id = v.vehicle_id
                   WHERE b.status IN ('สำเร็จ', 'ยกเลิก')'''
...
@staff.route('/bookings/<int:booking_id>')
@login_required
def view_booking(booking_id):
    """หน้ารายละเอียดการจองสำหรับพนักงาน"""
    try:
        cursor = get_cursor()
        ...
        cursor.execute('''
            SELECT b.*, 
                   ...
                   CONCAT(v.brand_name, ' ', v.model_name) as vehicle_info,
                   v.license_plate,
                   CONCAT(t.brand_name, ' ', t.model_name, ' ', t.size) as tire_info
            FROM bookings b
            JOIN customers c ON b.customer_id = c.customer_id
            JOIN vehicles v ON b.vehicle_id = v.vehicle_id
        ...
        return render_template('staff/view_booking.html', booking=booking)
        
    except Exception as e:
        print(f"Error in view_booking: {e}")
        flash('เกิดข้อผิดพลาดในการโหลดข้อมูลการจอง', 'error')
        return redirect(url_for('staff.dashboard'))
